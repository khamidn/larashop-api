...
class ShopController extends Controller{
    ...
    protected function getServices($data){
        ...
        $key="08364c05fce7cfbeb1de6e62996c0ba8";
    ...
    protected function validateCart($carts){
        ...
        foreach($carts as $cart){
            ...
            $book= Book::find($id);
            //$book = Book::find($id)->with('covers');
            $book = Book::where('id', $id);
            // $covers= BookCover::where('book_id',$id)->first();
            if($book){
                if($book->stock>0){
                    $safe_carts[$idx]['id'] = $book->id;  
                    $safe_carts[$idx]['title'] = $book->title;  
                    // $safe_carts[$idx]['covers'] = $book->covers;  
            ...
            try {
                ...
                if($order->save()){
                    ...
                    foreach($carts as $cart){
                        ...
                        if($book){
                            if($book->stock >= $quantity){
                                $total_harga += $book->price * $quantity;
                    ...
                    $total_bill = $total_harga + $service_cost;
